...
class config(Use):
    def configure_app(self, app, parser):
        ...
        parser.add_argument(
            "-5", "--G5", dest="model", const="G5", action="store_const", default=None
        )
...
class battery(scan):
    def main(self, args, app):
        battery = dict(
            level=self.dexcom.ReadBatteryLevel(), status=self.dexcom.ReadBatteryState()
        )
        return battery
...
class UpdateTime(scan):
    ...
    def upload_program(self, program):
        ...
        return dict(
            requested=dict(**program), result=result, offset=new_offset.total_seconds()
        )
...
class DescribeClocks(scan):
    ...
    def main(self, args, app):
        system = dict(
            offset=self.dexcom.ReadSystemTimeOffset().total_seconds(),
            utc=self.dexcom.ReadSystemTime(),
        )
        # system.update
        display = dict(
            offset=self.dexcom.ReadDisplayTimeOffset().total_seconds(),
...
* iter_glucose 2 - read last 2 records
    ...
    def main(self, args, app):
        ...
        candidates = itertools.takewhile(
            self.comparison, self.dexcom.iter_records(self.RECORD_TYPE)
        )
...
_EGVRecord = collections.namedtuple(
    "EGV",
    database_records.EGVRecord.BASE_FIELDS
    + database_records.EGVRecord.FIELDS
    + ["full_trend"],
)
...
_SensorRecord = collections.namedtuple(
    "Sensor",
    database_records.SensorRecord.BASE_FIELDS + database_records.SensorRecord.FIELDS,
)
...
class oref0_glucose(glucose):
    ...
    def configure_app(self, app, parser):
        ...
        parser.add_argument(
            "--no-raw", action="store_true", default=False, help="Skip raw data."
        )
        parser.add_argument(
            "--glucose",
            default=None,
    ...
    def get_glucose_data(self, params, args):
        ...
        return itertools.takewhile(
            self.comparison, self.dexcom.iter_records("EGV_DATA")
        )
    ...
    def get_sensor_data(self, params, args):
        if args.no_raw:
            ...
            if args.sensor:
                return [
                    SensorRecord(**fix_display_time(**item))
                    for item in json.load(argparse.FileType("r")(args.sensor))
                ]
            else:
                return itertools.takewhile(
                    self.comparison, self.dexcom.iter_records("SENSOR_DATA")
                )
    ...
    def main(self, args, app):
        ...
        template = dict(
            device="openaps://{}/{}".format(socket.gethostname(), self.device.name),
            type="sgv",
        )
        for egv, raw in itertools.zip_longest(iter_glucose, iter_sensor):
            ...
            if egv:
                ...
                item.update(
                    sgv=egv.glucose,
                    direction=self.trend_to_direction(trend, egv.trend_arrow),
                    **egv.to_dict(),
                )
...
class calibrations(glucose):
    ...
    RECORD_TYPE = "CAL_SET"
    TEXT_COLUMNS = (
        database_records.Calibration.BASE_FIELDS + database_records.Calibration.FIELDS
    )
