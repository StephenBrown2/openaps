...
def configure_app(app, parser):
    ...
    parser.add_argument(
        "reporter", choices=[r.__name__.split(".").pop() for r in reports]
    )
    commands = uses.UseDeviceCommands(
        app.parent.devices, parent=app, config=app.parent.config
    )
    app.usages = commands
...
def main(args, app):
    report = Report(
        report=args.report, device=args.device, reporter=args.reporter, use=args.use
    )
    task = (
        app.actions.selected(args)
        .usages.commands[args.device]
        .method.commands[report.fields["use"]]
    )
